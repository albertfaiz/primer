...
Pass either a registered name (str) or your own ``{key: multiplier}`` dict. A key
may be a **rule name** (scales that rule's activation) or a **capability name**
(e.g. ``interpretable``, ``native_categorical`` — scales that capability's
contribution and adds a standing preference, so it bites even when no rule fires;
``0.0`` zeroes a capability out, ``>1`` amplifies it).
"""
